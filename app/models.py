...
class Store(Base):
    ...
    store_password_encrypted = Column(Text, nullable=True)
...
class Chair(Base):
    ...
    maintenance_records = relationship(
        "Maintenance", back_populates="chair", cascade="all, delete-orphan"
    )
...
class Maintenance(Base):
    ...
    chair = relationship("Chair", back_populates="maintenance_records")
...
class Settings(Base):
    ...
    company_share_percentage = Column(Float, default=75)

    store_share_percentage = Column(Float, default=25)

    minimum_daily_revenue = Column(Float, default=500)

    target_daily_revenue = Column(Float, default=1000)
...
class Notification(Base):
    ...
    chair_id = Column(Integer, ForeignKey("chairs.id"))

    store_id = Column(Integer, ForeignKey("stores.id"))

    recipient_type = Column(String)  # store_owner / business_head
    ...
    channel = Column(String, default="pending")  # sms / whatsapp / email
